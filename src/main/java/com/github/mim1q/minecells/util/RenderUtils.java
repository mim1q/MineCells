...
public class RenderUtils {
  ...
  public static void drawBillboard(VertexConsumer consumer, MatrixStack matrices, int light, float width, float height, int argb) {
    drawBillboard(consumer, matrices, light, width, height, argb, OverlayTexture.DEFAULT_UV);
  }

  public static void drawBillboard(VertexConsumer consumer, MatrixStack matrices, int light, float width, float height, int argb, int overlay) {
    drawBillboard(consumer, matrices, light, width, height, 0.0F, 1.0F, 0.0F, 1.0F, argb, overlay);
  }
  ...
  public static void drawBillboard(VertexConsumer consumer, MatrixStack matrices, int light, float width, float height, float minU, float maxU, float minV, float maxV, int argb) {
    drawBillboard(consumer, matrices, light, width, height, minU, maxU, minV, maxV, argb, OverlayTexture.DEFAULT_UV);
  }

  public static void drawBillboard(VertexConsumer consumer, MatrixStack matrices, int light, float width, float height, float minU, float maxU, float minV, float maxV, int argb, int overlay) {
    float dx = width / 2F;
    float dy = height / 2F;

    drawBillboard(consumer, matrices, light, -dx, dx, -dy, dy, minU, maxU, minV, maxV, argb, overlay);
  }
