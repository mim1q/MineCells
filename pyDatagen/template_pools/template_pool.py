...
def generate_template_pool(elements: [(str, int)]):
    result = ""
    for element in elements:
        result += ELEMENT_TEMPLATE.replace("{{name}}", element[0]).replace("{{weight}}", str(element[1]))

    result = result[:-2]
    return TEMPLATE.replace("{{elements}}", result)
...
class TemplatePoolGenerator:
    ...
    def generate_prefixed(self, file_name: str, elements: [(str, int)], prefix: str):
        self.save_to_file(file_name, generate_template_pool([(prefix + "/" + element[0], element[1]) for element in elements]))
    ...
    def generate(self, file_name: str, elements: [(str, int)]):
        self.save_to_file(file_name, generate_template_pool(elements))
